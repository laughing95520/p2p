...
<div class="row">
    ...
    <div class="container">
        <div class="jumbotron">
            <h1 class="threed" align="center">成就梦想从这里开始</h1>
        </div>
...
<script>
    ...
    $("#btn-ljdk").on("click", function () {
        var money = $('#jkje').val();
        var day = $('#jkqx').val();
        var interestRate = ${loanItem.rate};
        var overdueInterestRate = ${loanItem.overRate};
        var stageCount = day;

        $.post("${pageContext.request.contextPath}/user/loan/apply.html", {
            money: money,
            day: day,
    ...
    $("#btn-profit").on("click", function () {
        ...
        //alert(jkje);
        if (jkje >${loanItem.moneyUp}) {
            layer.tips('贷款金额输入不能大于'+${loanItem.moneyUp}+
            '元', $('#jkje'), {tips: [3, '#53a0e3'], time: 2000}
        )
            ;
            $("#jkje").val('').focus();
            return false;
        }
        if (jkje <${loanItem.moneyDown}) {
            layer.tips('贷款金额输入不能小于'+${loanItem.moneyDown}+
            '元', $('#jkje'), {tips: [3, '#53a0e3'], time: 2000}
        )
            ;
            $("#jkje").val('').focus();
        ...
        if (nlv <= 0) {
            layer.tips('年利率输入不正确！', $('#nlv'), {tips: [3, '#53a0e3'], time: 2000});
            $("#nlv").val('').focus();
            return false;
        }
        if (parseFloat(jkqx) <= 0) {
            layer.tips('贷款期限输入不正确！', $('#jkqx'), {tips: [3, '#53a0e3'], time: 2000});
            $("#jkqx").val('').focus();
            return false;
        }

        if (parseInt(jkqx) >${loanItem.dayUp}) {
            layer.tips('期限不能超过'+${loanItem.dayUp}+
            '个月！', $('#jkqx'), {tips: [3, '#53a0e3'], time: 2000}
        )
            ;
            $("#jkqx").val('').focus();
            return false;
        ...
        if (parseInt(jkqx) <${loanItem.dayDown}) {
            layer.tips('期限至少'+${loanItem.dayDown}+
            '个月！', $('#jkqx'), {tips: [3, '#53a0e3'], time: 2000}
        )
            ;
            $("#jkqx").val('').focus();
